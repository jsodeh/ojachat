import { Menu, Globe, ChevronDown, Key, Video, Users, ShoppingCart } from "lucide-react";
...
const Sidebar = ({ isOpen, onToggle, isMobile = false, onNewChat, chatSessions, currentSessionId, onSessionSelect }: SidebarProps) => {
  ...
  const ChatList = () => (
    <div className="mt-4 flex flex-col gap-4">
      {Object.entries(groupedSessions).map(([timeframe, sessions]) => sessions.length > 0 && (
        <div key={timeframe}>
          <div className="px-3 py-2 text-xs text-gray-500">{timeframe}</div>
          {sessions.map((session) => (
            <div 
              ...
              className={cn(
                ...
                session.id === currentSessionId && "bg-token-sidebar-surface-secondary"
              )}
            >
    ...
    return (
      <Sheet>
        <SheetTrigger asChild>
          <button className="fixed top-4 left-4 z-40 h-10 rounded-lg px-2 text-token-text-secondary hover:bg-token-sidebar-surface-secondary">
            <Menu className="h-5 w-5" />
          </button>
        </SheetTrigger>
        <SheetContent side="left" className="w-80 bg-chatgpt-sidebar p-0 [&>button]:hidden">
